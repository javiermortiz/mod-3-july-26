...
Your goal in this practice is to add **JavaScript** code to an existing web page
(which already has working `html`, `css` and `js` files) in order to persist a
few settings the user made when the web page is refreshed, and even when the 
...
In your terminal, open that folder (`cd <folder-name>`), run the package install
(`npm install`), and start the express server (`npm start`). While this is
a front-end JavaScript project (NOT an **Express** project), it is helpful to 
run a web server to make it easier to debug with the **Developer Tools** in your
browser.

Open __public/main.js__ for editing in **VS Code**. This is the only file you
need to modify to complete this practice. The functions you need to complete
are found at the top.

Open your browser. Go to http://localhost:8081. Click on the theme buttons,
...
"Application". Notice the items on the left named "Local Storage" and "Session
Storage". You can open these and select the site you are running to see any
values stored there. (Probably none yet, but there will be soon!)
...
Your practice using cookies will be a variation of this one and begin with the 
same starter project. For easier reference for yourself in the future (such as,
while taking the assessment or coding larger projects), you may want to edit
__public/index.html__ to set the `<title>` and `<h1>` to "Practice Web Storage".
...
In the function named `storeTheme`, write the provided `themeName` into an item
in `localStorage`. You will need to choose an appropriate key for this item
as you set it.
...
However, it is likely that your code didn't work, if nothing changed.
...
If it exists, set it as the theme. Make sure you check that it's not empty or
missing. If not, you'll be making a bug for yourself in part C.

> HINT: There is already a function that does all the heavy lifting. You know
> it's working because the button clicks work. So take a look at what the
> buttons in the HTML are calling when they are clicked, or explore the 
> **Javascript** code to figure out which function causes the page to use a
> particular theme.
...
You can do this by clearing the local storage using the small "no" icon (circle 
...
* Part A: Implement the `storeName` function to set the passed in `displayName`
  as an item in `sessionStorage`.
* Part B: Implement the `restoreName` function to get the display name item from
  ...
  `<input>` field.
* Part C: Implement the `clearName` function to remove the display name item
  from `sessionStorage`.

> HINT: A function was already written to set the value of the input field.
> Explore, search, and/or read the **JavaScript** code in __public/main.js__ 
> to find it.
...
website for the project (http://localhost:8081). Notice the theme colors are 
showing and the button looks selected, while the display name is empty.
...
## Bonus Phase: Small performance enhancement

If you have a little extra time or would like a little more practice, then find
the `resetTheme` function (you'll have to make changes below the comment that
says "DO NOT EDIT BELOW THIS LINE"). Comment out the three lines under the 
"Remove selection styling from all buttons" comment. Then write some code to 
clear only the currently-selected button.

> HINT: Consider where the current selection is stored (per Phase 1, above).

You probably noticed this did not change the functionality. However, this 
improves efficiency (because fewer DOM updates are run), and greatly improves 
extensibility (because additional themes can be added with no JavaScript 
changes).
